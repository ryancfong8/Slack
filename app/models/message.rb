class Message < ApplicationRecord
  ...
  after_commit { MessageRelayJob.perform_later(self, channel) }
  after_save :reindex
  ...
  def has_reacted?(reaction, user)
    ...
    user_reactions_on_message = user.reactions.where(message_id: id)
  ...
  def as_indexed_json(_options = {})
    body_hash = {
      'body' => Sanitize.fragment(body).strip,
      'channel_members' => channel.members.map { |member| member.id }
    ...
    as_json(
      include: {
        ...
        channel: { only: %i[id name channel_type members channel_private],
                   include: {
                     members: { only: %i[id username] }
                   } }
      }
    ).merge(body_hash)
  end
